...
app.post('/api/generate', async (req, res) => {
    ...
    const lessonTypeGuide = resolvedLessonType === 'inquiry'
        ? `8. 교수·학습 활동은 **탐구질문 기반 흐름**으로 구성한다: 도입(탐구 질문 제시) → 전개1(탐구1: 자료 탐색·근거 찾기) → 전개2(탐구2: 적용·실행/문제 해결) → 전개3(공유: 주장·근거 발표 및 상호질문) → 정리(성찰: 처음 생각-바뀐 생각-이유).`
        : `8. 교수·학습 활동은 **일반형 흐름**으로 구성한다: 도입(학습 동기·과제 안내) → 전개1/2/3(핵심 기능 학습·적용·확인) → 정리(학습 내용 정리·다음 차시 예고). 탐구 질문은 보조적으로 활용하되, 활동명을 탐구1/탐구2로 강제하지 않는다.`;
    const lessonTypeActivityRule = resolvedLessonType === 'inquiry'
        ? `4-1. 활동명은 아래 구조를 기본으로 차시 맥락에 맞게 구체화하세요.
- 도입: 탐구 질문 열기/탐구 문제 인식
- 전개1: 탐구1(자료 탐색·근거 찾기)
- 전개2: 탐구2(적용·실행/문제 해결)
- 전개3: 공유(주장·근거 발표, 상호 질문)
- 정리: 성찰(처음 생각-바뀐 생각-이유, 다음 탐구 연결)`
        : `4-1. 활동명은 차시 목표와 핵심 활동이 드러나게 구체화하세요.
- 도입: 학습 동기 유발/학습 문제 확인
- 전개1: 핵심 개념·기능 이해
- 전개2: 적용·연습/실습
- 전개3: 확인·공유/피드백
- 정리: 배운 내용 정리·형성평가·다음 차시 연결`;
    const lessonTypeDetailRule = resolvedLessonType === 'inquiry'
        ? `- 도입은 탐구 질문을 제시하고 학습 문제를 인식하게 하세요.
- 전개1은 자료 탐색과 근거 찾기, 전개2는 적용·실행(또는 문제 해결), 전개3은 공유·발표·상호질문으로 구성하세요.
- 정리는 성찰 중심으로 구성하세요(무엇을 배웠는가 + 생각이 어떻게 바뀌었는가 + 왜 바뀌었는가).`
        : `- 도입은 학습 목표와 과제를 분명히 제시하세요.
- 전개는 이해→적용→확인의 단계가 드러나게 구성하세요.
- 정리는 핵심 정리, 학습 확인, 다음 차시 연결이 포함되게 구성하세요.`;
    const systemPrompt = `
...
- **교수·학습 활동**은 **도입 1개** + **전개 3개**(활동1·활동2·활동3, 실현 어려우면 2개) + **정리 1개**.
${lessonTypeDetailRule}
